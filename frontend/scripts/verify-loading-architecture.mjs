...
for (const file of sourceFiles) {
  ...
  if (
    /skeleton/i.test(relative) ||
    /(?:^|[^a-z])skeleton(?:[^a-z]|$)/i.test(source)
  ) {
  ...
  if (
    source.includes("data-placeholder") &&
    !source.includes("loadingPlaceholder(")
  ) {
    failures.push(
      `${relative}: placeholder nodes exist without Tailwind loading presentation`,
    );
  }
}

const loadingRoutes = files.filter((file) =>
  file.endsWith(`${path.sep}loading.tsx`),
);
for (const file of loadingRoutes) {
  ...
  if (
    ...
    /<(?:div|section|article|main)\b/.test(source)
  ) {
    failures.push(
      `${relative}: route loading file must delegate to the normal view in loading state`,
    );
  }
}

if (placeholderCount < 100)
  failures.push(`Only ${placeholderCount} real data placeholders found`);
if (placeholderStyleCount < 100)
  failures.push(
    `Only ${placeholderStyleCount} Tailwind placeholder styles found`,
  );
if (loadingStateCount < 50)
  failures.push(`Only ${loadingStateCount} loading-state hooks found`);
...
console.log(
  `- ${loadingRoutes.length} route loading files delegate to normal views`,
);
