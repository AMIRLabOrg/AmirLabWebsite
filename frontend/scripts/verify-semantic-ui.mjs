...
for (const token of [
  ...
  expect(
    ...
    `Public UI still contains decorative sequence token: ${token}`,
  );
}
...
expect(
  !/\{error\s*(?:&&|\?)/.test(bulkBar),
  "Bulk review bar must not own a persistent server-error panel.",
);
expect(
  bulkBar.includes("attentionCount"),
  "Bulk review bar must expose compact item-attention state.",
);
...
for (const file of itemIssueFiles) {
  expect(
    read(file).includes("ReviewIssueStamp"),
    `${file} must visually mark item-specific failures or attention states.`,
  );
}

const clientApi = read("src/lib/client-api.ts");
expect(
  clientApi.includes("safeStatusMessage"),
  "Client API must retain safe fallback messages.",
);
expect(
  clientApi.includes("ApiRequestError"),
  "Client API must expose structured safe errors.",
);
...
console.log(
  `Semantic UI contracts verified (${publicFiles.length} public surfaces, ${itemIssueFiles.length} item-action surfaces).`,
);
