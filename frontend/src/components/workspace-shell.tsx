...
import { ArrowUpRight, Bell, LogOut, PanelLeftClose, PanelLeftOpen } from "lucide-react";
    ...
    <div data-loading={(loading || !user) || undefined}>
      <div 
        className={cn(
          ...
          sidebarOpen ? "grid-cols-[260px_minmax(0,1fr)]" : "grid-cols-[64px_minmax(0,1fr)]"
        )}
      >
        <aside className="sticky top-0 flex h-screen min-w-0 flex-col overflow-y-auto overflow-x-hidden border-r border-line bg-paper pt-[.85rem] pb-4 max-[820px]:static max-[820px]:h-auto max-[820px]:w-full max-[820px]:overflow-visible max-[820px]:border-r-0 max-[820px]:border-b max-[820px]:pt-[.65rem] max-[820px]:pb-0">
                  ...
                  return (
                    <Link
                      className={cn(
                        ...
                        sidebarOpen ? "px-[.48rem]" : "justify-center px-0 relative",
          ...
          <div className={cn("mt-auto grid gap-[.15rem] pt-[.8rem] max-[820px]:hidden", sidebarOpen ? "px-[.85rem]" : "px-[.4rem]")}>
            <button title={!sidebarOpen ? "Log out" : undefined} className={cn("flex min-h-[38px] w-full cursor-pointer items-center gap-[.45rem] border-0 bg-transparent py-[.45rem] text-[.75rem] font-semibold text-danger transition-colors hover:text-danger-hover disabled:cursor-not-allowed disabled:opacity-55", sidebarOpen ? "justify-between px-[.7rem]" : "justify-center px-0")} disabled={loading || !user} onClick={() => setConfirmLogout(true)} type="button">
              {sidebarOpen ? (
                <span className="inline-flex items-center gap-[.45rem]"><LogOut aria-hidden="true" size={15} className="shrink-0" /> Log out</span>
              ) : (
                <LogOut aria-hidden="true" size={17} className="shrink-0" />
              )}
            </button>
            {sidebarOpen && (
              <Link className="flex min-h-[38px] items-center justify-between gap-[.45rem] px-[.7rem] py-[.45rem] text-[.75rem] text-ink-muted transition-colors hover:text-brand whitespace-nowrap" href="/" target="_blank">
                Public website <ArrowUpRight aria-hidden="true" size={15} className="shrink-0" />
              </Link>
            )}
          </div>
        </aside>
        <div className="grid min-w-0 grid-rows-[52px_minmax(0,1fr)] max-[820px]:grid-rows-[65px_minmax(0,1fr)]">
          <header className="sticky top-0 z-30 flex min-h-[52px] items-center justify-between border-b border-line bg-[color-mix(in_srgb,var(--canvas)_96%,transparent)] px-[clamp(1rem,2vw,1.8rem)] py-[.45rem]">
            ...
            <div className="flex items-center gap-[.55rem]">
              <Link aria-label={`${unreadCount} unread notification${unreadCount === 1 ? "" : "s"}`} className="relative inline-flex h-9 w-9 items-center justify-center rounded-control text-ink-muted transition-colors hover:bg-brand-faint hover:text-brand" href="/workspace/notifications">
