...
export function ApplicationReviewQueue() {
  ...
  useEffect(() => {
    ...
    void apiRequest<PaginatedResponse<ApplicationSummary>>(`/applications?${params}`, { method: "GET" })
      .then((response) => {
        if (!active) return;
        setResult(response);
        setError(undefined);
    ...
    <DataTableShell>
      <div className="grid min-w-0 grid-cols-[minmax(220px,1.4fr)_repeat(2,minmax(140px,.52fr))_minmax(220px,.8fr)_auto] items-end gap-[.8rem] rounded-panel border border-line bg-surface p-4 max-[980px]:grid-cols-2 max-[640px]:grid-cols-1">
        ...
        <FormField htmlFor="application-status" label="Status">
          <SelectControl id="application-status" onValueChange={(value) => { setLoading(true); setStatus(value); setPage(1); }} options={STATUSES.map((value) => ({ label: value === "ALL" ? "All statuses" : label(value), value }))} value={status} />
        </FormField>
        <FormField htmlFor="application-sort" label="Sort">
          <SelectControl id="application-sort" onValueChange={(value) => { setLoading(true); setSort(value); setPage(1); }} options={[{ label: "Newest", value: "NEWEST" }, { label: "Oldest", value: "OLDEST" }, { label: "Name", value: "NAME" }]} value={sort} />
        </FormField>
        ...
        <ButtonControl disabled={!filtered} onClick={clear}>Clear</ButtonControl>
      </div>
      ...
      {error && !result ? (
        <StatePanel action={{ label: "Retry", onClick: () => { setLoading(true); setReload((value) => value + 1); } }} body="The connection dropped. Nothing was lost; reconnect to continue." title="Could not load applications" variant="error" />
        ...
        <>
          ...
          <PaginationControls loading={loading} onPageChange={(nextPage) => { setLoading(true); setPage(nextPage); }} page={result?.page ?? page} pageSize={result?.pageSize ?? 20} total={result?.total ?? 0} totalPages={result?.totalPages ?? 1} />
        </>
      ) : (
        <StatePanel action={filtered ? { label: "Clear filters", onClick: clear } : undefined} body={filtered ? "Try a broader date range or remove an active filter." : "New applications will appear here after resume processing."} title={filtered ? "No matching applications" : "The application queue is clear"} variant={filtered ? "filtered" : "empty"} />
      )}
    </DataTableShell>
  );
}
