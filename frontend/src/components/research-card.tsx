...
export function ResearchCard({ item, loading = false, variant = "card" }: { item?: ResearchItem; loading?: boolean; variant?: "card" | "index" }) {
  const meta = item?.paper?.venue ?? item?.dataset?.license ?? item?.project?.status?.replaceAll("_", " ") ?? (loading ? "Loading metadata" : undefined);
  const typeLabel = item?.type?.toLowerCase() ?? (loading ? "loading" : "research");
  const title = item?.title ?? item?.paper?.citation ?? (loading ? "Research title is loading" : "Untitled research item");
  const summary = item?.summary ?? (loading ? "Research summary is loading" : undefined);
  ...
  return (
    <article aria-busy={loading || undefined} className={cn(index ? "grid min-h-0 grid-cols-[95px_minmax(0,1fr)_150px] gap-[.4rem] border-b border-line bg-transparent px-[.2rem] py-4 max-[640px]:grid-cols-[70px_minmax(0,1fr)]" : "flex min-h-[250px] flex-col")} data-loading={loading || undefined}>
      ...
      {summary ? <p className={cn(index && "col-start-2 mt-[.15rem] mb-0 text-[.72rem] leading-[1.55] text-ink-muted", loading && loadingPlaceholder(true, "text"))} data-placeholder={loading ? "text" : undefined}>{summary}</p> : null}
      {meta ? <span className={cn(index ? "col-start-3 row-start-1 text-right font-mono text-[.56rem] text-ink-muted max-[640px]:col-start-2 max-[640px]:row-auto max-[640px]:text-left" : "text-[.82rem] text-ink-muted", loading && loadingPlaceholder(true, "text", "short"))} data-placeholder={loading ? "text" : undefined} data-placeholder-width="short">{meta}</span> : null}
      {loading ? (
        <Link aria-disabled className={cn("inline-flex items-center gap-[.4rem] font-bold text-brand", index ? "col-start-3 row-start-2 self-end justify-self-end text-[.66rem] max-[640px]:col-start-2 max-[640px]:row-auto max-[640px]:justify-self-start" : "mt-auto pt-6 text-[.88rem]")} href="#" tabIndex={-1}>Loading <ExternalLink aria-hidden="true" className="opacity-10" size={15} /></Link>
