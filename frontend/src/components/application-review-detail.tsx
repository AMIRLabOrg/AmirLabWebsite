...
interface ReviewApplication {
  ...
  educationSummary: string | null; relevantLinks: string[]; status: string;
  extractedText: string | null; parsedResume: unknown; parseFeedback: string | null;
  position: { title: string }; createdAt: string; decisionReason: string | null;
}
type Decision = "ACCEPTED" | "REJECTED";

function ParsedResume({ value, loading = false }: { value: unknown; loading?: boolean }) {
  const resume = !loading && value && typeof value === "object" && !Array.isArray(value)
    ? value as { profile?: Record<string, unknown>; sections?: Record<string, unknown>; pageCount?: unknown; textLength?: unknown }
    : undefined;
  const profileEntries = loading
    ? [["Field", "Loading extracted value"], ["Field", "Loading extracted value"], ["Field", "Loading extracted value"]] as Array<[string, unknown]>
    : Object.entries(resume?.profile ?? {});

  if (!loading && !resume) return <p className="m-0 text-[.82rem] leading-[1.5] text-ink-muted">No structured browser extraction is available.</p>;

  return <div className="mt-4 grid border-t border-line" data-loading={loading || undefined}>
    {profileEntries.map(([key, item], index) => <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-[.35rem] py-3" key={`${key}-${index}`}><span className={cn("text-[.72rem] capitalize text-ink-muted", loadingPlaceholder(loading, "label"))} data-placeholder={loading ? "label" : undefined}>{key}</span><strong className={cn("text-[.82rem] [overflow-wrap:anywhere]", loadingPlaceholder(loading, "text", "long"))} data-placeholder={loading ? "text" : undefined} data-placeholder-width="long">{String(item ?? "Not detected")}</strong></div>)}
    {!loading && resume?.pageCount ? <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-[.35rem] py-3"><span className="text-[.72rem] capitalize text-ink-muted">Pages</span><strong className="text-[.82rem] [overflow-wrap:anywhere]">{String(resume.pageCount)}</strong></div> : null}
    {!loading && resume?.sections ? Object.entries(resume.sections).filter(([, lines]) => Array.isArray(lines) && lines.length).map(([name, lines]) => <section className="border-t border-line py-4" key={name}><h3 className="font-serif text-base capitalize">{name}</h3><ul className="mt-[.7rem] grid gap-[.45rem] pl-[1.2rem] text-[.8rem] leading-[1.5] text-ink-muted">{(lines as unknown[]).map((line, index) => <li key={index}>{String(line)}</li>)}</ul></section>) : null}
  </div>;
}
...
export function ApplicationReviewDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    void apiRequest<ReviewApplication>(`/applications/${id}`, { method: "GET" })
      .then((item) => { if (active) { setApplication(item); setError(undefined); } })
      .catch((caught: unknown) => { if (active) setError(caught instanceof Error ? caught.message : "Unable to load application."); })
  ...
  async function decide({ note, status }: { note?: string; status: Decision }) {
    ...
    router.push("/workspace/applications"); router.refresh();
  }

  if (!loading && !application) return <StatePanel action={{ label: "Retry", onClick: () => { setLoading(true); setReload((value) => value + 1); } }} body="The review record could not be retrieved." title={error ?? "Application not found"} variant="error" />;
  const canDecide = !loading && user?.role === "ADMIN" && application?.status === "NEEDS_REVIEW";
  const status = application?.status ?? "NEEDS_REVIEW";
  const relevantLinks = loading ? [undefined] : application?.relevantLinks ?? [];
  ...
  return <div className="grid min-w-0 gap-5" data-loading={loading || undefined}>
    ...
    <div className="grid grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)] items-start gap-4 max-[820px]:grid-cols-1">
      <section className="min-w-0 rounded-panel border border-line bg-surface p-6">
        ...
        <dl className="m-0 grid gap-0"><div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 border-t border-line py-[.8rem]"><dt className="text-[.78rem] font-[750] text-ink-muted">Email</dt><dd className={cn("m-0", loadingPlaceholder(loading, "text", "long"))} data-placeholder="text" data-placeholder-width="long">{application?.email ?? "loading@example.org"}</dd></div><div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 border-t border-line py-[.8rem]"><dt className="text-[.78rem] font-[750] text-ink-muted">Phone</dt><dd className={cn("m-0", loadingPlaceholder(loading, "text", "medium"))} data-placeholder="text" data-placeholder-width="medium">{application?.phone ?? (loading ? "Loading phone" : "Not provided")}</dd></div><div className="grid grid-cols-[120px_minmax(0,1fr)] gap-4 border-t border-line py-[.8rem]"><dt className="text-[.78rem] font-[750] text-ink-muted">Education</dt><dd className={cn("m-0", loadingPlaceholder(loading, "text", "full"))} data-placeholder="text" data-placeholder-width="full">{application?.educationSummary ?? (loading ? "Loading education" : "Not provided")}</dd></div></dl>
        <ParsedResume loading={loading} value={application?.parsedResume} />
        {relevantLinks.length ? <div className="grid gap-2 border-t border-line pt-4"><h3 className="font-serif text-base">Relevant links</h3>{relevantLinks.map((link, index) => <a className={cn("flex items-center gap-[.4rem] text-[.75rem] text-brand [overflow-wrap:anywhere]", loadingPlaceholder(loading, "text", "long"))} data-placeholder={loading ? "text" : undefined} data-placeholder-width="long" href={link ?? "#"} key={link ?? `link-loading-${index}`} rel="noreferrer" tabIndex={loading ? -1 : undefined} target={loading ? undefined : "_blank"}>{link ?? "Loading relevant link"}<ExternalLink aria-hidden="true" size={14} /></a>)}</div> : null}
      </section>
      <aside className="min-w-0 rounded-panel border border-line bg-surface p-6">
        ...
        <div className="mt-4 grid gap-2 rounded-panel border border-line bg-canvas p-4"><span className="font-mono text-[.64rem] uppercase tracking-[.06em] text-ink-muted">ATS assessment</span>{loading ? <p className={cn("m-0 text-[.8rem] leading-[1.55] text-ink-muted", loadingPlaceholder(true, "text", "full"))} data-placeholder="text" data-placeholder-width="full">Loading ATS assessment</p> : application?.status === "PARSE_FAILED" ? <SemanticStatus loading={loading} tone="error">The uploaded PDF could not be processed automatically.</SemanticStatus> : application?.status === "PARSING" ? <SemanticStatus loading={loading} tone="pending">Automatic CV processing is still running.</SemanticStatus> : <><SemanticStatus loading={loading} tone="success">Automatic CV processing completed.</SemanticStatus>{application?.parseFeedback ? <p className="m-0 text-[.8rem] leading-[1.55] text-ink-muted">{application.parseFeedback}</p> : null}</>}</div>
    ...
    {loading || canDecide ? (
      <div className="sticky bottom-3 z-[5] grid gap-4 rounded-panel border border-line bg-surface p-5">
        <ReviewActions
          ...
          actions={[
            {
              confirmDescription: application ? `Reject ${application.fullName} and email the reviewer feedback. This cannot be reviewed again.` : "Reject this application with reviewer feedback.",
              confirmLabel: "Reject application",
              confirmTitle: "Reject this application?",
              label: "Reject",
              notePlaceholder: "Give useful feedback when rejecting an application.",
              requiresNote: true,
              status: "REJECTED" as Decision,
              tone: "danger",
            },
            {
              confirmDescription: application ? `Accept ${application.fullName} and create an account pending setup. No access email will be sent automatically.` : "Accept this application and create its account record.",
