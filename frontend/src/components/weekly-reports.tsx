...
export function WeeklyReports() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      apiRequest<WeeklyReportContext>("/weekly-reports/current", {
        method: "GET",
      }),
      ...
      .then(([current, reports]) => {
        ...
        setProjectIds(
          current.report?.projects.map(({ projectId }) => projectId) ?? [],
        );
        setOutputIds(
          current.report?.outputs.map(({ outputId }) => outputId) ?? [],
        );
        setError("");
      })
  ...
  const editable =
    Boolean(context) &&
    (!currentReport ||
      ["DRAFT", "CHANGES_REQUESTED"].includes(currentReport.status));
  ...
  async function saveDraft() {
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "The weekly report could not be saved.",
      );
    } finally {
  ...
  async function submitReport() {
    ...
    try {
      ...
      if (!saved) return;
      const report = await apiRequest<WeeklyReport>(
        "/weekly-reports/current/submit",
        {
          method: "POST",
        },
      );
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "The weekly report could not be submitted.",
      );
    } finally {
      setWorking(undefined);
    }
  ...
  return (
    <WorkspaceSurface measure="reading">
      <WorkspaceHero
        ...
        meta={
          <span>
            {context ? reportWeek(context.weekStart) : "Current reporting week"}
          </span>
        }
        title="Weekly report"
      />
      ...
      {error ? (
        <StatePanel
          ...
          body={error}
          title={
            context ? "Report not saved" : "Could not load weekly reporting"
          }
          variant="error"
        />
      ) : null}
      ...
      <WorkspaceSplit>
        <WorkspacePanel
          action={
            currentReport ? (
              <Badge tone={statusTone(currentReport.status)}>
          ...
          >
            {currentReport?.reviewNote ? (
              <div className="rounded-small border-l-[3px] border-warning bg-warning-soft p-4 text-ink">
                <strong className="text-[.76rem] tracking-[.04em]">
                  Supervisor note
                </strong>
                <p className="mt-2 whitespace-pre-wrap leading-[1.55] text-ink-muted">
                  {currentReport.reviewNote}
                </p>
              </div>
            ) : null}
            <FormField
              htmlFor="weekly-accomplishments"
              label="Completed work and evidence"
            ...
            <FormField
              htmlFor="weekly-blockers"
              label="Blockers and decisions needed"
              labelClassName="text-[.78rem] font-semibold tracking-[.04em]"
            >
              <TextareaControl
                loading={!context}
            ...
            <fieldset
              ...
              disabled={!editable}
            >
              <legend className="mb-3 text-[.78rem] font-semibold tracking-[.04em]">
              ...
              {!context || context.projects.length ? (
                (
                  ...
                  <CheckboxControl
                    ...
                    id={
                      project
                        ? `weekly-project-${project.id}`
                        : `weekly-project-loading-${index}`
                    }
                  ...
                  >
                    <span
                      ...
                      data-loading={!project || undefined}
                    >
                      <strong
                        className={cn(
                          "text-[.86rem]",
                          loadingPlaceholder(!project, "text", "long"),
                        )}
                        data-placeholder={!project ? "text" : undefined}
                      ...
                      <small
                        className={cn(
                          "text-[.72rem] text-ink-muted",
                          loadingPlaceholder(!project, "label", "medium"),
                        )}
                        data-placeholder={!project ? "label" : undefined}
                        data-placeholder-width="medium"
                      >
                        {project
                          ? `${project.tasks.completed} completed · ${project.tasks.due} due · ${project.tasks.open} open`
                          : "Completed · due · open"}
                      </small>
                    </span>
                  </CheckboxControl>
                ...
                <WorkspaceEmpty>
                  No active project membership is available for reporting.
                </WorkspaceEmpty>
              )}
            </fieldset>
            ...
            >
              ...
              {!context || context.outputs.length ? (
                (
                  context?.outputs ?? Array.from({ length: 2 }, () => undefined)
                ).map((output, index) => (
                  <CheckboxControl
                    ...
                    id={
                      output
                        ? `weekly-output-${output.id}`
                        : `weekly-output-loading-${index}`
                    }
                  ...
                  >
                    <span
                      className="grid gap-1"
                      data-loading={!output || undefined}
                    >
                      <strong
                        className={cn(
                          "text-[.86rem]",
                          loadingPlaceholder(!output, "text", "long"),
                        )}
                        data-placeholder={!output ? "text" : undefined}
                        data-placeholder-width="long"
                      >
                        {output?.title ?? "Research output title"}
                      </strong>
                      <small
                        className={cn(
                          "text-[.72rem] text-ink-muted",
                          loadingPlaceholder(!output, "label", "short"),
                        )}
                        data-placeholder={!output ? "label" : undefined}
                ...
                <WorkspaceEmpty>
                  No authored or contributed paper or dataset is available yet.
                </WorkspaceEmpty>
              )}
            </fieldset>
            ...
            {editable || !context ? (
              ...
              <p className="m-0 text-[.8rem] text-ink-muted">
                This report is with a supervisor. It becomes editable if changes
                are requested.
              </p>
            )}
          </form>
        ...
        >
          ...
          >
            {(history ?? Array.from({ length: 3 }, () => undefined)).map(
              (report, index) => (
                <article
                  className="grid gap-3 border-t border-line px-6 py-5 first:border-t-0 max-[640px]:p-5"
                  key={report?.id ?? `report-loading-${index}`}
                >
                  <span className="flex items-center justify-between gap-3">
                    <strong
                      className={cn(
                        "text-[.86rem]",
                        loadingPlaceholder(!report, "text", "medium"),
                      )}
                      data-placeholder={!report ? "text" : undefined}
                      data-placeholder-width="medium"
                    >
                      {report ? reportWeek(report.weekStart) : "Reporting week"}
                    </strong>
                    <Badge
                      loading={!report}
                      tone={report ? statusTone(report.status) : "neutral"}
                    >
                      {report ? reportStatusLabel(report.status) : "Draft"}
                    </Badge>
                  </span>
                  <small
                    className={cn(
                      "text-[.7rem] text-ink-muted",
                      loadingPlaceholder(!report, "label", "long"),
                    )}
                    data-placeholder={!report ? "label" : undefined}
                    data-placeholder-width="long"
                  >
                    {report
                  ...
                  <p
                    className={cn(
                      "m-0 line-clamp-3 text-[.8rem] leading-[1.5] text-ink-muted",
                      loadingPlaceholder(!report, "text", "full"),
                    )}
                    data-placeholder={!report ? "text" : undefined}
                    data-placeholder-width="full"
                  >
                    {report?.accomplishments ??
                      "Research activity and progress summary."}
                  </p>
                </article>
              ),
            )}
            {history?.length === 0 ? (
              <WorkspaceEmpty>
                Your first submitted report will appear here.
              </WorkspaceEmpty>
            ) : null}
          </div>
        </WorkspacePanel>
