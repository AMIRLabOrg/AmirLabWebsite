...
function peopleGroup(person: Person): PeopleGroup {
  ...
  if (role.includes("founder") || role.includes("research director")) return "founder";
...
function personRole(person: Person): string {
  return person.roleTitle ?? person.rank?.replaceAll("_", " ") ?? "AmirLab member";
}
...
export function PeopleDirectory({
  ...
  const loadingGroups = GROUPS.slice(0, 3).map((group) => ({ ...group, members: [] as Person[] }));
...
function Founder({ person, loading = false }: { person?: Person; loading?: boolean }) {
  ...
  return (
    <section className="border-t border-line-strong pt-[.8rem]" data-loading={loading || undefined}>
      ...
      <article className="grid grid-cols-[minmax(220px,320px)_minmax(0,1fr)_minmax(180px,240px)] items-stretch gap-[clamp(1.4rem,3.3vw,3.4rem)] max-[1000px]:grid-cols-[minmax(210px,300px)_minmax(0,1fr)] max-[720px]:grid-cols-[120px_minmax(0,1fr)] max-[480px]:grid-cols-1">
        ...
        <div className="grid min-w-0 content-center">
          ...
          <p className={cn("mt-4 line-clamp-4 max-w-[700px] text-[.82rem] leading-[1.65] text-ink-muted max-[720px]:hidden", loadingPlaceholder(loading, "text", "full"))} data-placeholder={loading ? "text" : undefined}>
            {person?.biography ?? "Research biography and institutional context are being loaded for this profile."}
          </p>
        ...
        <aside className="border-l border-line pl-[1.2rem] pt-[.4rem] font-mono text-[.56rem] leading-[1.8] tracking-[.055em] text-ink-muted max-[1000px]:hidden">
          <strong className="block font-medium text-ink">FOUNDER RECORD / PRIMARY RESEARCH LEAD</strong>
          <span className="block">Profile authority is drawn from the lab registry.</span>
          <span className="block">Project and publication links remain canonical.</span>
        </aside>
      </article>
    </section>
  );
...
function MemberCollection({ members, loading = false }: { members: Person[]; loading?: boolean }) {
  ...
  return (
    <>
      ...
      {hasMore ? (
        <button aria-expanded={expanded} className="mt-[.9rem] inline-flex cursor-pointer items-center gap-[.35rem] border-0 border-b border-brand bg-transparent px-0 py-1 text-[.68rem] font-semibold text-brand" onClick={() => setExpanded((current) => !current)} type="button">
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        odd ? "border-r border-line pr-[1.4rem] max-[720px]:border-r-0 max-[720px]:pr-[.4rem]" : "pl-[1.4rem] max-[720px]:pl-0",
      )}
      href={href}
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className={cn("m-0 block overflow-hidden text-ellipsis whitespace-nowrap text-[.7rem] leading-[1.4] text-ink-muted", loadingPlaceholder(loading, "text", "short"))} data-placeholder={loading ? "text" : undefined}>
          {person ? personRole(person) : "Research role"}
        </p>
        {loading || (person?.headline && person.headline !== person.roleTitle) ? (
          <small className={cn("mt-[.18rem] block overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[.56rem] leading-[1.4] text-ink-muted", loadingPlaceholder(loading, "text", "long"))} data-placeholder={loading ? "text" : undefined}>
            {person?.headline ?? "Research area and affiliation"}
          </small>
        ) : null}
      </div>
      <MoveUpRight aria-hidden="true" className={cn("text-ink-faint group-hover:text-brand", loading && "opacity-[.12]")} data-loading-icon={loading || undefined} size={19} />
    </Link>
  );
}
