...
export function PositionList({ positions = [], loading = false }: { positions?: Position[]; loading?: boolean }) {
  ...
  const tabs = [
    ...
    ...[...categories.entries()].map(([value, count]) => ({ count, label: rankLabel(value), value })),
  ];
  const visible = loading
    ? Array.from({ length: 3 }, () => undefined)
    : positions.filter((position) => category === "ALL" || position.positionType === category);
  ...
  return (
    ...
    >
      <div className="flex items-end justify-between border-t border-line-strong pt-[.7rem] max-[640px]:flex-col max-[640px]:items-start">
        <div><p className="m-0 mb-4 font-[var(--font-sans)] text-[.75rem] font-extrabold uppercase tracking-[.12em] text-brand">Current opportunities</p><h2 className="mt-[.1rem] font-serif text-[1.9rem] font-medium tracking-[-.03em]" id="current-opportunities">Open roles</h2></div>
        <span className={cn("font-mono text-[.68rem] text-ink-muted", loadingPlaceholder(loading, "text", "short"))} data-placeholder={loading ? "text" : undefined}>{loading ? "00 available" : `${positions.length} available`}</span>
      </div>
      {!loading && tabs.length > 2 ? (
        <TabsControl ariaLabel="Position categories" className="overflow-x-auto border-b border-line" onValueChange={setCategory} options={tabs} value={category} />
      ) : null}
      {visible.length ? (
        <div className="grid">
        ...
        <StatePanel body="New roles will appear here when the lab begins accepting applications." title="No positions are open right now" />
      )}
    </section>
  );
}
