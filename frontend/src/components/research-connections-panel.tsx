...
function outputOnly<T extends { researchItem?: { type: string }; type?: string }>(
  item: T,
): boolean {
...
export function ResearchConnectionsPanel() {
  ...
  if (!authLoading && staff) {
    return (
      <section className="mt-6 grid gap-6 rounded-panel border border-line bg-surface p-[clamp(1.2rem,3vw,2rem)]" aria-labelledby="research-admin-title">
        <div>
          <p className="m-0 mb-4 font-[var(--font-sans)] text-[.75rem] font-extrabold uppercase tracking-[.12em] text-brand">Research administration</p>
          <h2 className="mt-[.35rem] font-serif text-[clamp(1.8rem,4vw,2.8rem)]" id="research-admin-title">Paper and dataset register</h2>
          <p className="mt-3 max-w-[720px] text-[.86rem] leading-[1.6] text-ink-muted">Staff create records on behalf of registered members. Imported and newly submitted records remain in the verification queue until contributor identities and source evidence are reviewed.</p>
        </div>
        <div className="flex flex-wrap gap-3">
          <ButtonLink href="/workspace/research" variant="secondary">Open papers & datasets review</ButtonLink>
        </div>
      </section>
    );
  }
  return (
    <section className="mt-6 grid gap-6 rounded-panel border border-line bg-surface p-[clamp(1.2rem,3vw,2rem)]" aria-labelledby="research-connections-title" data-loading={loading || undefined}>
      <div className="flex items-end justify-between gap-4 max-[640px]:flex-col max-[640px]:items-start">
        <div>
          <p className="m-0 mb-4 font-[var(--font-sans)] text-[.75rem] font-extrabold uppercase tracking-[.12em] text-brand">Entity relationships</p>
          <h2 className="mt-[.35rem] font-serif text-[clamp(1.8rem,4vw,2.8rem)]" id="research-connections-title">Research connections</h2>
        </div>
      ...
      {loading || connections.length ? (
        <div className="grid border-t border-line">
          {(loading ? Array.from({ length: 2 }, () => undefined) : connections).map((connection, index) => {
            ...
            <article className="flex items-center justify-between gap-4 border-b border-line py-4 max-[640px]:flex-col max-[640px]:items-start" key={researchItem ? `${researchItem.id}:${displayName}` : `connection-loading-${index}`}>
              ...
              {loading || researchItem?.canonicalUrl ? (
                <a className={cn("inline-flex items-center gap-[.3rem] text-[.75rem] text-brand", loadingPlaceholder(loading, "label"))} aria-disabled={loading} data-placeholder={loading ? "label" : undefined} href={researchItem?.canonicalUrl ?? "#"} rel="noreferrer" tabIndex={loading ? -1 : undefined} target={loading ? undefined : "_blank"}>
        ...
        <p className="m-0 text-[.82rem] leading-[1.5] text-ink-muted">No verified output connections yet.</p>
      )}
      ...
      <form className="grid grid-cols-[minmax(220px,1fr)_minmax(220px,1fr)_auto] items-end gap-[.8rem] border-t border-line pt-5 max-[640px]:grid-cols-1" onSubmit={search}>
        <FormField htmlFor="connection-search" label="Find a published output to claim">
        ...
        <FormField htmlFor="connection-evidence" label="Supporting URL (recommended)">
          <InputControl
      ...
      {outputResults.length ? (
        <div className="grid gap-3">
          {outputResults.map((item) => (
            <article className="grid gap-4 border border-line p-4" key={item.id}>
              <div>
                <span className="font-mono text-[.62rem] uppercase text-brand">{item.type.toLowerCase()}</span>
                <h3 className="mt-1 font-serif text-[1.2rem]">{item.title ?? "Untitled research output"}</h3>
              </div>
              <div className="grid">
