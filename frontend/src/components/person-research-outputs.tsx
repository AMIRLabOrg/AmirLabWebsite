...
export function PersonResearchOutputs({
  ...
  return (
    <section
      ...
      className="border-b border-line bg-surface py-[clamp(3.5rem,7vw,6rem)]"
    ...
    >
      <PublicShell className="grid gap-12">
        <div className="max-w-[620px]">
          ...
          <h2
            className="my-[.65rem] mb-4 font-serif text-[clamp(2.3rem,5vw,4.2rem)] leading-[.95]"
            id="verified-research-title"
          >
            Research outputs
          </h2>
          <p className="m-0 leading-[1.65] text-ink-muted">
            ...
            <section className="grid gap-5" key={type}>
              ...
              <div
                className={
                  type === "PAPER"
                    ? "grid"
                    : "grid grid-cols-3 gap-px border border-line bg-line max-[980px]:grid-cols-2 max-[640px]:grid-cols-1"
                }
              >
              ...
              {!loading && items.length > PREVIEW_SIZE ? (
                <button
                  className="mt-[.9rem] inline-flex w-fit cursor-pointer items-center border-0 bg-transparent p-0 text-[.75rem] font-bold text-brand hover:text-brand-hover"
                  onClick={() =>
                    setExpanded((current) => ({
                      ...current,
                      [type]: !current[type],
