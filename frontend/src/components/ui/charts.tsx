...
export function LineChart({
  ...
  const x = (index: number) =>
    ...
    (labels.length === 1
      ? 0
      : (index / (labels.length - 1)) * (plot.right - plot.left));
  const y = (value: number) =>
    plot.bottom - (value / maximum) * (plot.bottom - plot.top);
  ...
  return (
    <div className="grid gap-3 px-6 pb-6 pt-5 max-[560px]:px-3">
      ...
      <ChartLegend
        series={series.map(({ color, label }) => ({ color, label }))}
      />
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-[minmax(150px,.8fr)_minmax(150px,1fr)] items-center gap-6 p-6 max-[560px]:grid-cols-1">
      <div className="relative mx-auto w-full max-w-[190px]">
        ...
        <span className="absolute left-1/2 top-1/2 grid -translate-x-1/2 -translate-y-1/2 text-center">
          <strong className="font-mono text-[1.45rem] font-medium">
            {total}
          </strong>
          <small className="text-[.66rem] text-ink-muted">Total tasks</small>
        </span>
      </div>
      <ChartLegend
        stacked
        series={segments.map(({ color, label, value }) => ({
          color,
          label,
          value,
        }))}
...
function ChartLegend({
  ...
  series: Array<{ color: string; label: string; value?: number }>;
  stacked?: boolean;
}) {
  return (
    <div
      className={
        stacked
          ...
          : "flex flex-wrap justify-center gap-x-5 gap-y-3"
      }
    >
      {series.map(({ color, label, value }) => (
        <span
          className={`inline-flex items-center gap-2 text-[.7rem] text-ink-muted ${stacked ? "border-b border-line pb-2" : ""}`}
          key={label}
        >
          <svg aria-hidden="true" height="7" viewBox="0 0 7 7" width="7">
            <circle cx="3.5" cy="3.5" fill={color} r="3.5" />
          </svg>
          {label}
          {value === undefined ? null : (
            <strong className="ml-auto font-mono font-medium text-ink">
              {value}
            </strong>
          )}
        </span>
