  ...
  return (
    <>
      <PageIntro
        aside={
          <IntroRegister
            loading={loading}
            items={[
              { label: "Published", value: loading ? "00" : list.length },
              { label: "Type", value: isDataset ? "Dataset" : "Project" },
              {
                label: "Related",
                value: isDataset ? "Projects / papers" : "People / outputs",
              },
      ...
      <section
        ...
        className="mx-auto grid w-full max-w-[var(--public-wide)] grid-cols-[minmax(0,1fr)_minmax(250px,300px)] items-start gap-[clamp(1.5rem,3vw,3rem)] px-[clamp(1rem,3.2vw,3rem)] pt-8 pb-20 max-[900px]:grid-cols-1 max-[640px]:px-4 max-[640px]:pt-[1.4rem] max-[640px]:pb-16"
      ...
      >
        ...
        <aside className="sticky top-[82px] grid gap-[.6rem] border-t border-line-strong max-[900px]:static max-[900px]:grid-cols-[180px_minmax(0,1fr)] max-[900px]:pt-4 max-[640px]:grid-cols-1">
          <div className="h-[180px] overflow-hidden border-b border-line max-[900px]:row-span-3 max-[900px]:h-[150px] max-[640px]:hidden">
            <MotionScene
              className="h-[200px] w-full opacity-70"
              variant={isDataset ? "dataset" : "project"}
            />
          </div>
          <p className="m-0 font-mono text-[.66rem] font-semibold tracking-[.105em] text-brand uppercase max-[900px]:col-start-2 max-[640px]:col-start-1">
            {isDataset ? "Dataset details" : "Project details"}
          </p>
          <h2 className="m-0 font-serif text-[1.55rem] font-medium tracking-[-.03em] max-[900px]:col-start-2 max-[640px]:col-start-1">
            {isDataset
              ? "Version, license, and access information."
              : "Objectives, milestones, team members, and outputs."}
          </h2>
          <p className="m-0 text-[.72rem] leading-[1.65] text-ink-muted max-[900px]:col-start-2 max-[640px]:col-start-1">
            {isDataset
              ? "Each dataset page shows the information supplied for access and reuse, together with related research when available."
              : "Each project page brings its public description, progress, collaborators, updates, and linked research together."}
          </p>
        </aside>
      </section>
    </>
